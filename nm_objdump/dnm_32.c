...
int m32(int fd, data32_t *d, Elf32_Sym *sym)
{
	...
	uint32_t	n = -1,
				...
				offset = 0,
				sto = 0;
	Elf32_Shdr	sh_strtab, sobj, strtab;
	...
	else if (st_shndx == SHN_ABS)
	{
		if (ELF32_ST_BIND(sym->st_info) == STB_WEAK)
			c = ((sym->st_other == STV_DEFAULT) || (sym->st_other == STV_PROTECTED)) ? 'A' : 'a';
		else
	...
	{
		if (ELF32_ST_BIND(sym->st_info) == STB_WEAK)
			c = ((sym->st_other == STV_DEFAULT) || (sym->st_other == STV_PROTECTED)) ? 'W' : 'w';
		else
			c = (ELF32_ST_BIND(sym->st_info) == STB_GLOBAL) ? 'T' : 't';
	}
	if ((ELF32_ST_TYPE(sym->st_info) == STT_OBJECT) || (ELF32_ST_TYPE(sym->st_info) == STT_NOTYPE))
	{
		...
		do {
			...
			if (read(fd, buf + pos, 1) != 1)
				return (1);
		} while (buf[pos] && (pos < 1024));
		if (ELF32_ST_TYPE(sym->st_info) == STT_OBJECT)
		{
			if (ELF32_ST_BIND(sym->st_info) == STB_WEAK)
				c = ((sym->st_other == STV_DEFAULT) || (sym->st_other == STV_PROTECTED)) ? 'V' : 'v';
			else if (!strcmp(buf, ".bss"))
				c = (ELF32_ST_BIND(sym->st_info) == STB_LOCAL) ? 'b' : 'B';
			else if (!strcmp(buf, ".rodata") || !strcmp(buf, ".interp") || !strcmp(buf, ".init") || !strcmp(buf, ".fini"))
				c = (ELF32_ST_BIND(sym->st_info) == STB_LOCAL) ? 'r' : 'R';
			else if (!strcmp(buf, ".text") || !strcmp(buf, ".plt") || !strcmp(buf, ".text.startup"))
		...
		{
			if (!strcmp(buf, ".text") || !strcmp(buf, ".plt"))
			{
				if (ELF32_ST_BIND(sym->st_info) == STB_WEAK)
					c = ((sym->st_other == STV_DEFAULT) || (sym->st_other == STV_PROTECTED)) ? 'W' : 'w';
				else
			...
			else if (!strcmp(buf, ".data") || !strcmp(buf, ".jcr") || !strcmp(buf, ".ctors") || !strcmp(buf, ".dtors") || !strcmp(buf, ".got"))
			{
				if (ELF32_ST_BIND(sym->st_info) == STB_WEAK)
					c = ((sym->st_other == STV_DEFAULT) || (sym->st_other == STV_PROTECTED)) ? 'V' : 'v';
				else
	...
	{
		if (ELF32_ST_BIND(sym->st_info) == STB_WEAK)
			c = ((sym->st_other == STV_DEFAULT) || (sym->st_other == STV_PROTECTED)) ? 'C' : 'c';
		else
			c = (ELF32_ST_BIND(sym->st_info) == STB_GLOBAL) ? 'C' : 'c';
	}
	else if (ELF32_ST_TYPE(sym->st_info) == STT_SECTION)
		c = (ELF32_ST_BIND(sym->st_info) == STB_GLOBAL) ? 'S' : 's';
	else if (ELF32_ST_TYPE(sym->st_info) == STT_LOPROC || ELF32_ST_TYPE(sym->st_info) == STT_HIPROC)
		c = 'p';
	else
		return (1);
	...
	if (read(fd, &sh_strtab, sizeof(Elf32_Shdr)) != sizeof(Elf32_Shdr))
		return (1);
	strtab_off = (d->endianness) ? bswap_32(sh_strtab.sh_offset) : sh_strtab.sh_offset;
	...
	(st_shndx == SHN_UNDEF) ? (printf("         %c %s\n", c, name_buf)) : (printf("%08x %c %s\n", st_value, c, name_buf));
	return (0);
}
